...
@Component
public class TextLoader extends AbstractLoader<TextConfiguration, TextDto> {
    ...
    @Override
    protected TextDto createDto(final TextConfiguration config) {
    ...
    private List<Line> createStructure(final TextConfiguration config) {
        ...
        Line currentLine = new Line(config.getFont().fontDto.spaceWidth, config.getFontSize(), config.getMaxLineLength());
        ...
        for (char c : chars) {
            ...
            if (ascii == SPACE_ASCII) {
                ...
                if (!added) {
                    ...
                    currentLine = new Line(config.getFont().fontDto.spaceWidth, config.getFontSize(), config.getMaxLineLength());
                    currentLine.attemptToAddWord(currentWord);
                }
                currentWord = new Word(config.getFontSize());
                continue;
            }
            Character character = config.getFont().fontDto.characterMap.get(ascii);
    ...
    private TextDto createQuadVertices(final TextConfiguration config, final List<Line> lines) {
        double cursorX = 0f;
        double cursorY = 0f;
        ...
        for (Line line : lines) {
            ...
            for (Word word : line.words) {
                ...
                cursorX += config.getFont().fontDto.spaceWidth * config.getFontSize();
            }
            cursorX = 0;
            cursorY += lineHeight;
        }
        return new TextDto(listToArray(vertices), listToArray(textureCoords));
    }
