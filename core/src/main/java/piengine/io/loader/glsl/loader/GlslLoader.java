package piengine.io.loader.glsl.loader;
...
@Component
public class GlslLoader extends AbstractLoader<String, GlslDto> {
    ...
    @Override
    protected GlslDto createDto(final String key) {
        final String vertexSource = tryToReadShader("vertex", key);
        final String tessControlSource = tryToReadShader("tessellation/control", key);
        final String tessEvalSource = tryToReadShader("tessellation/evaluation", key);
        final String geometrySource = tryToReadShader("geometry", key);
        final String fragmentSource = tryToReadShader("fragment", key);

        return new GlslDto(vertexSource, tessControlSource, tessEvalSource, geometrySource, fragmentSource);
    }
